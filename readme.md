...
When an event-sourced actor needs [Orleans journaling](https://learn.microsoft.com/en-us/dotnet/orleans/grains/event-sourcing/) 
semantics, add `[Journaled]` alongside `[Actor]`:
...
> [!NOTE]
> There's never any dynamic dispatch. Message routing is a compile-time `switch` on the 
