...
When the user requests creation of a cloud actor, ask the following clarifying questions: 

- Whether to use structured IDs or not (give an example related to the users's domain, if available)
- Whether to use event sourcing (explain why it may or may not make sense depending on the scenario)

When the user requests creation or inclusion of the cloud actor hosting, ask whether to use/configure 
Streamstone for event sourcing (if any cloud actors in the solution actually use event sourcing).